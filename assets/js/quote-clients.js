...
/**
 * Abrir modal de crear cliente desde presupuestos
 */
function openQuoteCreateClientModal() {
 ...
 */
function closeQuoteCreateClientModal() {
  const modal = document.getElementById('create-client-modal');
  if (modal) {
...
/**
 * Guardar nuevo cliente desde modal de presupuestos
 */
async function handleQuoteSaveClient(event) {
  event.preventDefault();
  
  // Deshabilitar botón con animación de carga
  const saveBtn = document.getElementById('quote-client-save-btn') || event.target.querySelector('button[type="submit"]');
