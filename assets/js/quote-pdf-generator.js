/**
 * Sistema de generación de PDF para presupuestos - Multipágina
 * Utiliza jsPDF para crear presupuestos profesionales con soporte de varias páginas
 * Basado en InvoicePDFGenerator, con terminología de presupuestos
 */
...
class QuotePDFGenerator {
  ...
  drawPageFooter() {
    ...
    this.doc.text(this.quoteNumber || 'Presupuesto', this.margin, this.pageHeight - 10);
  ...
  drawContinuationHeader() {
    ...
    this.doc.text(this.quoteNumber ? 'Presupuesto ' + this.quoteNumber + ' (cont.)' : 'Continuación', this.margin, this.currentY);
    this.currentY += 8;
  }
  ...
  generatePDF(quoteData) {
    ...
    this.currentY = this.margin;
    this.pageNumber = 1;
    this.quoteNumber = quoteData.quote?.number || '';

    // Página 1: Cabecera completa
    this.drawHeader(quoteData.issuer);
    this.drawClientAndQuoteInfo(quoteData.client, quoteData.quote);

    // Tabla de conceptos (con paginación)
    this.drawConceptsTable(quoteData.concepts);

    // Gastos suplidos
    if (quoteData.expenses && quoteData.expenses.length > 0) {
      this.checkPageBreak(20);
      this.drawExpensesTable(quoteData.expenses);
    }

    // Resumen financiero
    this.checkPageBreak(60);
    this.drawFinancialSummary(quoteData.summary);

    // Métodos de pago
    if (quoteData.paymentMethods && quoteData.paymentMethods.length > 0) {
      this.checkPageBreak(20);
      this.drawPaymentMethods(quoteData.paymentMethods);
    }

    // Observaciones
    if (quoteData.observations && quoteData.observations !== 'on' && quoteData.observations.trim()) {
      this.checkPageBreak(20);
      this.drawObservations(quoteData.observations);
    }
  ...
  drawClientAndQuoteInfo(client, quote) {
    ...
    this.doc.text('PRESUPUESTO', rx, this.currentY);
    this.doc.setFontSize(8);
    var info = [];
    if (quote.series) info.push({ l: 'Serie', v: quote.series });
    if (quote.number) info.push({ l: 'Nº presupuesto', v: quote.number });
    if (quote.reference) info.push({ l: 'Referencia', v: quote.reference });
    if (quote.issueDate) info.push({ l: 'Fecha', v: quote.issueDate });
    if (quote.dueDate) info.push({ l: 'Validez', v: quote.dueDate });
    if (quote.paymentTerms) info.push({ l: 'Condiciones', v: quote.paymentTerms });
  ...
  drawFinancialSummary(summary) {
    ...
    this.doc.text('Total presupuesto', rx, this.currentY);
