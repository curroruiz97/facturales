    ...
    <tr class="border-b border-bgray-200 dark:border-darkblack-400">
      ...
      <td class="py-5 pr-6">
        <div class="flex flex-col">
          <p class="text-sm font-semibold text-bgray-900 dark:text-white">${escapeHtml(invoiceNumber)}</p>
          <p class="text-xs text-bgray-600 dark:text-bgray-50">Serie ${escapeHtml(draft.invoice_series)}</p>
        </div>
      </td>
...
function openDeleteModal(draftId) {
  ...
  const invoiceNumber = draft ? (draft.invoice_number || 'Sin número') : 'Sin número';
