/**
 * drafts-page.js
 * Script para la página de borradores de facturas
 */
...
async function loadDrafts() {
  try {
    ...
    // Obtener borradores
    const result = await window.getInvoices({ status: 'draft' });
        ...
        <td colspan="5" class="py-12 text-center">
          <div class="flex flex-col items-center gap-4">
            ...
            <div>
              ...
              <p class="mt-1 text-sm text-bgray-600 dark:text-bgray-50">Guarda una factura como borrador para verla aquí</p>
            </div>
          </div>
        </td>
      </tr>
...
function createDraftRow(draft) {
  const invoiceNumber = draft.invoice_number || 'Sin número';
  const clientName = draft.client_name || 'Sin cliente';
  const clientNif = draft.invoice_data?.client?.nif || '-';
  const issueDate = window.formatDate ? window.formatDate(draft.issue_date) : draft.issue_date;
  const dueDate = draft.due_date ? (window.formatDate ? window.formatDate(draft.due_date) : draft.due_date) : '-';
  const totalAmount = window.formatCurrency ? window.formatCurrency(draft.total_amount, draft.currency) : `${draft.total_amount} ${draft.currency}`;
    ...
    <tr class="border-b border-bgray-200 dark:border-darkblack-400">
      ...
      <td class="py-5 pr-6">
        <div class="flex flex-col">
          <p class="text-sm font-semibold text-bgray-900 dark:text-white">${invoiceNumber}</p>
          <p class="text-xs text-bgray-600 dark:text-bgray-50">Serie ${draft.invoice_series}</p>
        </div>
      </td>
      ...
      <td class="py-5 pl-6 text-right">
        <div class="inline-flex items-center gap-2">
          ...
          <!-- Botón Eliminar -->
          <button
            onclick="openDeleteModal('${draft.id}', '${invoiceNumber}')"
            class="inline-flex h-9 w-9 items-center justify-center rounded-lg bg-bgray-100 transition hover:bg-bgray-200 dark:bg-darkblack-500 hover:dark:bg-darkblack-400"
            type="button"
            title="Eliminar"
          >
 ...
 */
function openDeleteModal(draftId, invoiceNumber) {
...
function handleSearchDrafts(searchTerm) {
  ...
  const filtered = allDrafts.filter(draft => {
    return (
      draft.invoice_number?.toLowerCase().includes(term) ||
      draft.client_name?.toLowerCase().includes(term) ||
      draft.invoice_data?.client?.nif?.toLowerCase().includes(term)
    );
  });
...
document.addEventListener('DOMContentLoaded', () => {
  console.log('✅ drafts-page.js inicializado');
  loadDrafts();
});
