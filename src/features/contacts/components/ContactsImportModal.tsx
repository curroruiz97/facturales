import type { ContactsImportPreview } from "../domain/contacts-import";

interface ContactsImportModalProps {
  open: boolean;
  fileName: string | null;
  preview: ContactsImportPreview | null;
...
export function ContactsImportModal(props: ContactsImportModalProps): import("react").JSX.Element | null {
  const { open, fileName, preview, parseError, importing, onClose, onPickFile, onConfirmImport } = props;
  if (!open) return null;

  return (
    <div className="pilot-modal" role="dialog" aria-modal="true" aria-label="Importar contactos">
      ...
      <div className="pilot-modal__content">
        <div className="pilot-modal__header">
          <h3 className="text-lg font-bold">Importar contactos (CSV/XLSX)</h3>
        ...
        <div className="pilot-panel">
          <p className="text-sm opacity-80">Selecciona un archivo con columnas de nombre/razon social e identificador.</p>
                ...
                <div className="mt-2 max-h-36 overflow-auto">
                  <ul className="space-y-1 text-xs">
                    {preview.invalidRows.slice(0, 12).map((row) => (
                      <li key={`import-error-${row.rowIndex}`} className="rounded border border-red-200 px-2 py-1 text-red-700">
