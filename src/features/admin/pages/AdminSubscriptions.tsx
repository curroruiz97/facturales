...
export function AdminSubscriptions(): import("react").JSX.Element {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    const result = await AdminUserService.getUsers(page, 20, search);
    setData(result);
    setLoading(false);
  }, [page, search]);
    ...
    <AdminLayout>
      <div className="space-y-6">
        ...
        <div className="flex items-center gap-3.5">
          ...
          <div>
            <h2 className="text-xl font-bold tracking-tight text-slate-900 dark:text-white">Suscripciones</h2>
            <p className="text-sm text-slate-500 dark:text-slate-400">Vista de suscripciones de todos los usuarios</p>
          </div>
        </div>
            ...
            <table className="w-full text-left text-sm">
              <thead>
                <tr className="border-b border-slate-200 bg-slate-50/80 dark:border-slate-700 dark:bg-slate-800/80">
                  ...
                  <th className="hidden px-5 py-3.5 text-xs font-semibold uppercase tracking-wider text-slate-500 dark:text-slate-400 md:table-cell">Clientes</th>
                ...
                {loading ? (
                  ...
                  (data?.users ?? []).map((user) => (
                    <tr key={user.id} className="group border-b border-slate-100 transition-colors last:border-0 hover:bg-orange-50/40 dark:border-slate-700/50 dark:hover:bg-slate-700/30">
                      <td className="px-5 py-3.5 font-medium text-slate-900 dark:text-white">{user.email}</td>
                      ...
                      <td className="hidden px-5 py-3.5 font-medium tabular-nums text-slate-700 dark:text-slate-300 md:table-cell">{user.clients_count}</td>
                      ...
                      <td className="px-5 py-3.5">
                        <Link to={`/admin/users/${user.id}`} className="inline-flex items-center gap-1 rounded-lg border border-orange-200 bg-orange-50 px-3 py-1.5 text-xs font-semibold text-orange-700 transition-colors hover:bg-orange-100 dark:border-orange-700/40 dark:bg-orange-900/20 dark:text-orange-400 dark:hover:bg-orange-900/30">
                          Gestionar
                        </Link>
                      </td>
                    </tr>
